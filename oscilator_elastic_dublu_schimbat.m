...
TR=0; % dinamica in timp real - 1

% Parametrii fizici ai sistemului:

m1=0.5; m2=0.05; % kg; masele corpurilor

ka=10; kb=10; kc=10; % N/m; constantele elastice ale resorturilor

La=0.5; Lb=0.5; Lc=0.5; % m; lungimile resorturilor nedeformate

% Deplasari si viteze initiale:

eta10=0.2; eta20=-0.2; % m
...
for i=2:N-1
    ...
    eta1(i+1)=2*eta1(i)-eta1(i-1)-dt^2*(omega11^2*eta1(i)-omega12^2*eta2(i)) -(2*cfr*v1(i-1)*dt^2);

    eta2(i+1)=2*eta2(i)-eta2(i-1)-dt^2*(omega22^2*eta2(i)-omega21^2*eta1(i)) -(2*cfr*v2(i-1)*dt^2);
